...
public class HeterogeneousMaxHolder {
    ...
    public <T extends Comparable<? super T>> T put(Class<T> key, T value) {
        if (!maxValues.containsKey(key) || value.compareTo((T) maxValues.get(key)) > 0) {
            T oldMax = (T) maxValues.put(key, value);
            return oldMax; // Devolver el antiguo valor máximo
        } else {
            return value;
        }
    }
    ...
    public <T> T put(Class<T> key, T value, Comparator<? super T> comparator) {
        if (!maxValues.containsKey(key) || comparator.compare(value, (T) maxValues.get(key)) > 0) {
            T oldMax = (T) maxValues.put(key, value);
            return oldMax; // Devolver el antiguo valor máximo
        } else {
            return value;
