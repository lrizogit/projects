...
public class CrazyGenerics {
    ...
    static class CollectionUtil {
         ...
         */
        public static <T extends BaseEntity> boolean hasNewEntities(Collection<? extends BaseEntity> entities) {
            return entities.stream().anyMatch(entity -> entity.getUuid() == null); // todo: refactor parameter and implement method
        }
        ...
        public static <T extends BaseEntity> boolean hasDuplicates(List<T> entities, T targetEntity) {
            ...
            long count = entities.stream()
                    .filter(entity -> entity.getUuid() != null && entity.getUuid().equals(targetEntity.getUuid()))
                    .count();

            return count > 1; // todo: update method signature and implement it
        }
        ...
        public static <T> Optional<T> findMax(Iterable<T> elements, Comparator<? super T> comparator) {
            ...
            if (!iterator.hasNext()) {
                return Optional.empty(); // No elements in the iterable
            }
        ...
        public static <T extends BaseEntity> T findMostRecentlyCreatedEntity(Collection<T> entities) {
            return findMax(entities, CREATED_ON_COMPARATOR)
                    .orElseThrow(() -> new NoSuchElementException("Unable to find most recently created entity"));
        }
        /**
